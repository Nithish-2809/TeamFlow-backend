...
const removeBoardMember = async (req, res) => {
  try {
    ...
    io.to(`board_${boardId}`).emit("tasks:unassigned-bulk", {
      boardId,
      userId,
    });
...
const leaveBoard = async (req, res) => {
  try {
    ...
    if (!membership) {
      return res
        .status(404)
        .json({ msg: "You are not the member of this board!" });
    }

    if (membership.isAdmin) {
      return res
        .status(400)
        .json({ msg: "You cannot leave the board as you are admin." });
    }
...
const makeBoardAdmin = async (req, res) => {
  ...
  try {
    ...
    await Board.updateOne({ _id: boardId }, { leader: userId }, { session });
